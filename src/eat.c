...
char *stoptingle = "%s stops tingling and your sense of smell returns to normal.";
	 ...
	 /* case PM_SANDESTIN: */
	    ...
	    case PM_DISENCHANTER:
		{
			...
			if ((otmp->spe <
			    ...
			    if (!Blind)
				pline_The("blue glow converges on your %s, and disappears.", xname(otmp));
			    else
			    ...
			    if (!Blind)
				pline_The("blue glow disappears.");
			}
		    }
		}
		break;
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	if(tin.tin->spe != 1) {
		...
		if (isstoneorslime(tin.tin->corpsenm)) {
		    pline("It could be very dangerous!");
		    dangerfood = TRUE;
		} else if (!vegetarian(&mons[tin.tin->corpsenm])) {
		    if (Role_if(PM_MONK)) {
			pline("It could be unhealthy.");
			dangerfood = TRUE;
		    } else if (!u.uconduct.unvegetarian) {
			pline("It could be foul and unfamiliar to you.");
			dangerfood = TRUE;
		    }
		}
	    }
	    Sprintf(buf, "Eat it%s?", dangerfood ? " anyway" : "");
	    if (yn(buf) == 'n') {
...
//	if ((cadaver && !vegetarian(&mons[mnum]) || material == FLESH) &&
