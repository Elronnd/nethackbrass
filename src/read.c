...
doread()
{
	...
	if (IS_GRAVE(levl[u.ux][u.uy].typ))
	    if (yn("Do you want to read an epitaph on the grave?") == 'y') {
		read_engr_at(u.ux, u.uy);
		return (0);
	    }

	scroll = getobj(readable, "read", getobj_filter_read);
	if(!scroll) return(0);
...
int curse_bless;
{
	...
	if (obj->oclass == WAND_CLASS) {
		...
		Your("%s cracks, and explodes!",xname(obj));
		losehp(d(4,8), "exploding orb", KILLED_BY_AN);
		useup(obj);
		break;
	    case HORN_OF_PLENTY:
	    case BAG_OF_TRICKS:
...
getchargableobj()
{
	return getobj(all_count, "charge", getobj_filter_charge);
}
...
struct obj *obj;
{
	...
	if(!on) {
		...
		if (obj->oartifact == ART_SUNSWORD)
			pline("A brilliant white-hot light bursts from the sword!");
		else
