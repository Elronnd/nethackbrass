...
STATIC_DCL int FDECL(fire_gun, (struct obj *));
...
dofire()
{
	...
	if(check_capacity((char *)0)) return(0);
	if (uwep && is_gun(uwep)) {
		if (!fire_gun(uwep)) return FALSE;
		if (u.twoweap && is_gun(uswapwep))
			if (!fire_gun(uswapwep)) return FALSE;
		return TRUE;
	}
	if (!uquiver) {
		if (!flags.autoquiver) {
...
#ifdef MONSTEED
		   && !mon->msteed /* msteed cannot be tamed easily */
#endif
...
struct obj *otmp;
{
	...
	if (Underwater) {
		Your("gun isn't an underwater gun.");
		return(0);
	}
	...
	if (mtmp) {
		...
		if (IS_SINK(typ)) {
			pline("Suddenly the bullet turns down and hits the sink!");
		} else {
