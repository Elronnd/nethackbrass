...
long reason;
{
	...
	if (cured & SUFFO_NECK) {
		...
		pline("Now you can drink water...");
	    } else if (StrangledBy & SUFFO_NOAIR)
		pline("Unfortunately still you get no air.");
	    else
		You("can breathe more easily!");
	} else if (cured & SUFFO_WATER) {
	    /* water elemental */
	    if (StrangledBy & SUFFO_NECK)
		You("are still strangled.");
	    else if (StrangledBy & SUFFO_NOAIR)
		You("still get no air!");
	    else if (Amphibious && !breathless(youmonst.data))
		You("can breathe now.");
	    else if (Breathless)
		You_feel("no longer choked.");
	    else
...
nh_timeout()
{
	...
	if (u.uspellit) {
	    ...
	    if (age == 0) {
		extinguish_torch();
		if (!Blind) pline_The("magical light around you goes away.");
	    }
	    if (age == 50) {
		change_light_source_range(LS_MONSTER, &youmonst, 2);
		if (!Blind) Your("magic torch is getting dim.");
	    }
	}
