...
/*	Pre-movement adjustments	*/
	...
	if (mtmp->msteed ||
	     (mtmp->mriding &&
		(mtmp->mchild->mtrapped || !mtmp->mchild->mcanmove ||
		 mtmp->mchild->msleeping || mtmp->mchild->permove))) {
	    dontmove = 1;
	}
#endif /*MONSTEED*/
...
toofar:
	...
	if (wants_steed(mdat) && !mtmp->mriding && !mtmp->mconf &&
	    !mtmp->mstun && !rn2(3)) {
	    struct monst *mstd;
		...
		if (cansee(mstd->mx, mstd->my)) {
		    ...
		    pline("%s mounts on %s.", Monnam(mtmp), stdnam);
		}
...
#ifdef MONSTEED
	    || (mtmp->mriding &&
		(mtmp->mchild->mconf || (appr == 1 && mtmp->mchild->mflee)))
#endif
	   ) appr = 0;
	else {
#ifdef GOLDOBJ
