...
dosit()
{
 ...
 in_water:
		...
		if (u.uevent.uplayed_tune &&
		    xdnstair == 0 && ydnstair == 0) {
			mkstairs(41, 11, 0, (struct mkroom *)0);
			newsym(41,11);
			if (!Blind) You("find secret stairs!");
			u.uevent.ufound_town = 1;
			return(1);
		}
	    }*/ /*ctown*/
	    if (rnd(6) > 4)  {
...
rndcurse()			/* curse a few inventory items at random! */
{
	...
	if ( rn2(100) < tmp ) {
	    if (tmp > 20) {
		You(mal_aura, "you..");
		Your("imposing air defeats the black aura!");
	    }
	    return;
	}
