	...
	register struct monst *mtmp;
{
	...
	if (mtmp->msteed) {
	    ...
	    if (!tmp) {
		if (canseemon(mtmp))
		    pline("%s bucks wildly!", Monnam(mtmp));
		mdismount_steed(mtmp);
		return 1;
	    } else if (tmp == 1) {
		if (canseemon(mtmp))
		    pline("%s tries to throw off the rider!", Monnam(mtmp));
		return 1;
	    }
	}
#endif /*MONSTEED*/
	...
	register struct	attack *mattk;
{
	...
	if (mdef->msteed) mdismount_steed(mdef);
#endif
	...
	if ((status & MM_AGR_DIED) && (status & MM_DEF_DIED)) {
...
#ifdef MONSTEED
	    if (MON_AT(dx, dy)) {	/* rider died but steed survives */
		place_monster(magr, ax, ay);	/* put back aggressor */
		newsym(ax, ay);
		return status;
	    }
#endif
	    ...
	    newsym(dx, dy);
#ifdef MONSTEED
	    /* if mrider is swallowed and survives, fall off at last */
	    if (mdef->mriding) mdismount_steed(mdef);
#endif
	}
	...
	register struct attack	*mattk;
{
 ...
 post_stone:		if (mdef->mhp > 0) return 0;
		...
		if (obj) {
			...
			if (vis) {
				...
				pline("%s steals %s from %s!", buf,
				    onambuf, mdefnambuf);
				if (vanish_ether_obj(otmp, magr, "steal"))
				    otmp = (struct obj *)0;
		...
		break;
	    case AD_DRWN:
		if(vis) pline("%s drowns.", Monnam(mdef));
		tmp = mdef->mhp;
		/* Use up amulet of life saving */
		if (!!(obj = mlifesaver(mdef))) m_useup(mdef, obj);
		break;
