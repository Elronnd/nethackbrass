...
//#ifdef AUTOTHRUST
	if(u.ulasttgt != mtmp) u.ulasttgt = mtmp;	/* you targetted it */
//#endif
	...
	if (malive && !distant && !Upolyd && !is_caitiff(mtmp) &&
	    !mtmp->wormno && /* for long worm tail */
	    (twcan || !Stunned) && !multi) {
	    if (u.twoweap) {						/* two-weapon combat */
#ifdef WIZARD
		if (wizard) pline("[2W]");
#endif
...
uchar hflg;
{
	...
	if (Fumbling && !rn2(3)) {
		You("swing wildly and miss %s.", mon_nam(mon));
		return (malive);
	}
	if(!*mhit) {
	    missum(mon, uattk);
...
#ifdef MONSTEED
			if (jousting && mon->mriding) pdismount = 100;
#endif /*MONSTEED*/
		    }
#endif
#ifdef MONSTEED
		    /* dismount attack */
		    if (mon->mriding && obj == uwep) {
...
#undef useup_eggs
	...
	if (valid_weapon_attack) {
	    ...
	    if (u.twoweap) use_skill(P_TWO_WEAPON_COMBAT, 1);
	}
...
/*	These only affect you if they still live */
	...
	if(malive && !mon->mcan && rn2(3)) {
		...
		if (!distant || !otmp || is_metallic(otmp)) {
		    ...
		    You("are jolted with electricity%s", Shock_resistance ? "." : "!");
		    mdamageu(mon, tmp);
		}
		break;
	      default:
...
pline("<%d>",dmg);
			...
			if (msg) {
			    if (seeit) {
				You("barely hit %s.", mon_nam(mtmp));
				pline("%s %s blocks your attack.", mon_its(mtmp),
