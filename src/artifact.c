...
aligntyp alignment;	/* target alignment, or A_NONE */
{
	...
	if (n) {		/* found at least one candidate */
	    ...
	    otmp->oartifact = m;
	    if (a->material) change_material(otmp, a->material);
	    artiexist[m] = TRUE;
...
doinvoke()
{
    ...
    obj = getobj(invoke_types, "invoke", getobj_filter_invoke);
    if(!obj) return 0;
...
boolean
vorpalize_weapon(otmp,omrg)
     struct obj *otmp; /* otmp should be named, non-unique object */
     struct obj *omrg; /* omrg should be non-unique object */
{
  struct artifact *a = &artilist[ART_CUSTOM_MADE_WEAPON];
  boolean vorpalized = TRUE;

  if (artiexist[ART_CUSTOM_MADE_WEAPON]) return(FALSE);
  if (otmp) {
    if (omrg) {
      switch (omrg->otyp) {
	...
	case WAN_FIRE:
		...
		a->spfx |= SPFX_ATTK;
		a->attk.adtyp = AD_FIRE;
		a->attk.damd = (omrg->spe > 0) ? (omrg->spe+1)/2 : 0;
		break;
	case WAN_COLD:
		if (get_material(otmp) == SILVER) {
			return(FALSE);
	...
	case WAN_LIGHTNING:
		...
		a->spfx |= SPFX_ATTK;
		a->attk.adtyp = AD_ELEC;
		a->attk.damd = (omrg->spe > 0) ? (omrg->spe+1)/2 : 0;
		break;

	case RIN_POISON_RESISTANCE:	a->defn.adtyp = AD_DRST; a->spfx |= SPFX_DEFN; break;
	case RIN_FIRE_RESISTANCE:	a->defn.adtyp = AD_FIRE; a->spfx |= SPFX_DEFN; break;
	case RIN_COLD_RESISTANCE:	a->defn.adtyp = AD_COLD; a->spfx |= SPFX_DEFN; break;
	case RIN_SHOCK_RESISTANCE:	a->defn.adtyp = AD_ELEC; a->spfx |= SPFX_DEFN; break;

	case RIN_INVISIBILITY:	a->inv_prop = INVIS;	break;
	case RIN_LEVITATION:	a->inv_prop = LEVITATION;	break;
	case RIN_CONFLICT:	a->inv_prop = CONFLICT;		break;
	case RIN_STEALTH:	a->spfx |= SPFX_STLTH;		break;
	case RIN_WARNING:	a->spfx |= SPFX_WARN;		break;
	case RIN_SEARCHING:	a->spfx |= SPFX_SEARCH;		break;
	case RIN_TELEPORT_CONTROL:
				a->spfx |= SPFX_TCTRL;		break;
	case RIN_GAIN_STRENGTH:
	case RIN_GAIN_CONSTITUTION:
	case RIN_INCREASE_DAMAGE:
		a->attk.damd = (omrg->spe > 0) ? omrg->spe : 0;
		break;
	case RIN_INCREASE_ACCURACY:
		a->attk.damn = (omrg->spe > 0) ? omrg->spe : 0;
		break;
  ...
  if (vorpalized) {
    a->attk.damn += (otmp->owt < 50 ) ? (50-(otmp->owt))/10+1 : 0; /* light weapons get some hit bonus */
    a->attk.damd += (otmp->spe > 0) ? otmp->spe : 0;
    a->otyp = otmp->otyp;
    a->alignment = u.ualign.type;
    a->role = u.umonster;
    Strcpy(pl_weapon, ONAME(otmp));
    a->name = pl_weapon;
    otmp->quan = 1;
    otmp->spe = 0;
    otmp->oartifact = ART_CUSTOM_MADE_WEAPON;
    otmp->oeroded = 0;
    otmp->oeroded2 = 0;
    otmp->opoisoned = 0;
    otmp->greased = 0;
    artiexist[ART_CUSTOM_MADE_WEAPON] = TRUE;
    pline("AD:%d H:%d D:%d Df:%d", a->attk.adtyp, a->attk.damn, a->attk.damd, a->defn.adtyp);
