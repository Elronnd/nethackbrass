...
struct obj *wep;
{
	...
	if (!wep) {
		...
		You_feel("this kind of weapon does not fit you at all.");
...
dowield()
{
	...
	if (!(wep = getobj(wield_objs, "wield", getobj_filter_wield)))
		/* Cancelled */
		return (0);
	else if (wep == uwep) {
...
dowieldsubweapon()	/* when user do #twoweapon with uwep && !uswapwep */
{
	...
	/* Prompt for a new weapon */
	if (!(wep = getobj(wield_objs, "wield as your secondary weapon", getobj_filter_wieldsub)))
		/* Cancelled */
		return (0);
	else if (wep == uwep) {
	    You("are already wielding that as your main weapon!");
	    return (0);
	} else if (wep == uswapwep) {
...
dowieldquiver()
{
	...
	if (!(newquiver = getobj(slinging ? bullets : ready_objs, "ready",
...
//	    /* protection might be gone */
//	    if (target->oerodeproof && !rn2(6)) {
//		if (victim == &youmonst)
//		    Your("%s no longer protected.", aobjnam(target, "are"));
//		else if (vismon)
//		    pline("%s's %s no longer protected.", Monnam(victim),
//			aobjnam(target, "are"));
//		target->oerodeproof = FALSE;
//	    } else if (flags.verbose || !(target->oerodeproof && target->rknown)) {
//		if (victim == &youmonst)
...
//		target->oeroded++;
//	} else {
//	    if (!rn2(5) && !target->oartifact) {
//		if (victim == &youmonst) {
//		    Your("%s apart!", aobjnam(target, "break"));
//		    remove_worn_item(target);
//		    useup(target);
//		} else if (vismon) {
//		    long unwornmask;
//		    pline("%s's %s apart!",
//			  Monnam(victim), aobjnam(target, "break"));
//		    if ((unwornmask = target->owornmask) != 0L) {
//			victim->misc_worn_check &= ~unwornmask;
//			if (target->owornmask & W_WEP)
//			    setmnotwielded(victim,target);
//			target->owornmask = 0L;
//			update_mon_intrinsics(victim, target, FALSE);
//		    }
//		    if (unwornmask & W_WEP)		/* wielded weapon is broken */
//			possibly_unwield(victim);
//		    else if (unwornmask & W_ARMG)	/* worn gloves are broken */
//			mselftouch(victim, (const char *)0, TRUE);
//		    m_useup(victim, target);
//		}
//	    } else
//	    if (flags.verbose) {
//		if (victim == &youmonst)
