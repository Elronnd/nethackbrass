...
const char *name;	/* if null, then format `obj' */
{
	...
	if(u.uac + tlev <= rnd(20)) {
		...
		if (is_acid && Acid_resistance)
			...
			/* so far a shield is always available... */
			if (dam) dam = reduce_damage(dam, DAMCAN_RANDOM, 1);
...
//	    int dam, hitv;
	...
	/* The polearms related code is moved into mattacku() */
//	    if (canseemon(mtmp)) {
//		onm = xname(otmp);
...
struct obj *ogun;
{
	...
	m_throw(mtmp, mtmp->mx, mtmp->my, sgn(tbx), sgn(tby),
		distmin(mtmp->mx, mtmp->my, mtmp->mux, mtmp->muy), oblt);
	nomul(0);
	return TRUE;
}
