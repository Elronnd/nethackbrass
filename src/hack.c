...
collision:
		if (Rocketskate
		    ...
		    char buf[BUFSZ];
		    pline("%s!  You crash into %s!",
			bang[rn2(4)], placenam(tmpr->typ));
		    Sprintf(buf, "crashed into %s", placenam(tmpr->typ));
		    losehp(rnd(4), buf, NO_KILLER_PREFIX);
		    return FALSE;
		}
...
domove()
{
	...
	if(u.uswallow) {
		...
		if (mtmp) {
			...
			if (flags.run && mtmp->mriding &&
			    ((!Blind && mon_visible(mtmp->mchild) &&
			      ((mtmp->mchild->m_ap_type != M_AP_FURNITURE &&
				mtmp->mchild->m_ap_type != M_AP_OBJECT) ||
			       Protection_from_shape_changers)) ||
			     sensemon(mtmp->mchild))) {
				nomul(0);
				flags.move = 0;
				return;
			}
	...
	if(mtmp) {
...
#ifdef MONSTEED
	    if (!mseen && mtmp->mriding && canspotmon(mtmp->mchild))
		mseen = mtmp->mchild;
#endif /*MONSTEED*/
	    nomul(0);
		...
		if(mtmp->m_ap_type && !Protection_from_shape_changers
		    ...
		    pline("Pardon me, %s.", m_monnam(mseen));
		else
		    You("move right into %s.", mon_nam(mseen));
		return;
	    }
	...
	if(u.utrap) {
		if(u.utraptype == TT_PIT) {
		    ...
		    if(--u.utrap) {
			if(flags.verbose) {
			    char *predicament = "stuck in the mud";
#ifdef STEED
			    if (u.usteed)
			    	Norep("%s is %s.",
