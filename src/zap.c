		...
		case AD_FIRE:
			...
			case TOOL_CLASS:
				...
				if (!objects[obj->otyp].oc_merge &&
					...
					obj->oeroded++;
					pline("Your %s catches fire and smoulders!", xname(obj));
				    }
				}
			    } else skip++;
			    break;
...
int damval;
{
	...
	switch (oc1) {
		...
		if (uamul) {
			...
			if ( newtmp>100 ) {
				if (Blind) You_feel("your medallion breaks in pieces!");
				else Your("medallion glows brightly, and breaks in pieces!");
				otmp = uamul;
				Amulet_off();
				useup(otmp);
				break;
			} else if ( /*oldtmp<90 &&*/ newtmp>=90 )
				if (!Blind) Your("medallion gives a faint glimmer of light!");
				else Your("medallion vibrates violently!");
			else if ( oldtmp<75 && newtmp>=75 )
				Your("medallion vibrates unexpectedly.");
		...
		switch (oc2) {
			...
			if (uarmc) {
				...
				if ( newtmp>100 ) {
					if (Blind)
					    You_feel("your %s is gone away!", cloak_simple_name(uarmc));
					else
					...
					Your("%s is about to come apart!", cloak_simple_name(uarmc));
				else if ( oldtmp<75 && newtmp>=75 )
					Your("%s becomes slightly shabby.", cloak_simple_name(uarmc));
			...
			if (uarms && prop == REFLECTING) {	/* shield of reflection */
				...
				newtmp = oldtmp + damval;
				if ( newtmp>100 ) {
					if (!Blind) Your("shield is completely tarnished.");
					else You_feel("your shield is somewhat helpless.");
					otmp = uarms;
					Shield_off();
					otmp->otyp = SHIELD;
					otmp->odamaged = 0;
				    ...
				    if (!Blind) Your("shield looks much duller!");
				    else Your("shield shivers!");
				} else if ( oldtmp<75 && newtmp>=75 ) {
				    if (!Blind) Your("shield looks slightly dull.");
				    else Your("shield shivers slightly.");
				}
			...
			if (uarm && Is_dragon_armor(uarm)) {	/* gray or silver dragon */
				...
				if ( newtmp>100 ) {
				    if (isdm) {
					Your("%s seems to lose its power.", buf);
					otmp = uarm;
					Armor_off();
					...
					Your("%s crumble and fall apart!", buf);
					otmp = uarm;
					Armor_off();
					useup(otmp);
				    }
