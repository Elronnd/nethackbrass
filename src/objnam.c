...
register struct obj *obj;
{
	...
	switch (obj->oclass) {
		...
		if (is_poisonable(obj) && obj->opoisoned)
			Strcpy(buf, "poisoned ");
		if (obj->madeof) {
			Strcat(buf, materialnm2[get_material(obj)]);
		}
	    case VENOM_CLASS:
	    case TOOL_CLASS:
		if (typ == LENSES
		...
		if(nn) {
			...
			if(is_boots(obj))
				...
				Strcpy(buf,armor_simple_name(obj));
			Strcat(buf, " called ");
			Strcat(buf, un);
		} else	Strcat(buf, dn);
		break;
		...
		Sprintf(buf, "%sheavy iron ball",
			((int)obj->owt > (int)ocl->oc_weight) ? "very " : "");
		break;
	    case POTION_CLASS:
...
#define SPINACH 2
	...
	for(;;) {
		...
		if (!strncmpi(bp, "an ", l=3) ||
			...
			if (!strncmpi(bp+l, "angel", 5)) break;
			else blessed = 1;
...
//				goto corpse;
	...
	if (strncmpi(bp, "nurse cap", 9)) /* not the "nurse" monster */
	if (strncmpi(bp, "nurse uniform", 13)) /* not the "nurse" monster */
...
struct obj *armor;
{
    ...
    if (armor) {
	t = armor->otyp;
	if ( is_robe(armor) ) return "robe";
	switch (t) {
	case MAID_DRESS:
	    return "dress";
	case NURSE_UNIFORM:
	    return (objects[t].oc_name_known &&
			armor->dknown) ? "uniform" : "dress";
	default:
	    break;
	}
    }
...
uchar typ;
{
	...
	else if (IS_DEADTREE(typ)) what = "a dead tree";
	else if (IS_ROCK(typ)) what = "a rock";
	...
	else if (typ == IRONBARS) what = "iron bars";
