...
STATIC_OVL int FDECL(xy2dir, (int,int));
 ...
 */
/* clockwise */
char vx[8] = {  0,  1,  1,  1,  0, -1, -1, -1 };
char vy[8] = { -1, -1,  0,  1,  1,  1,  0, -1 };
...
int x, y;
{
	...
	if (x>=0 && y<0) {
	  if (x < -y) {
	    ...
	    dir = 2.0 - ((double)(-y) / (double)(x));
	  }
	} else if (x>=0 && y>=0) {
	  if (y < x) {
	    dir = 2.0 + ((double)(y) / (double)(x));
	  } else {
	    dir = 4.0 - ((double)(x) / (double)(y));
	  }
	} else if (x<0 && y>=0) {
	  if (-x < y) {
	    dir = 4.0 + ((double)(-x) / (double)(y));
	  } else {
	    dir = 6.0 - ((double)(y) / (double)(-x));
	  }
	} else {
	  if (-y < -x) {
	    dir = 6.0 + ((double)(-y) / (double)(-x));
	  } else {
	    dir = 8.0 - ((double)(-x) / (double)(-y));
	  }
	}
	return ((int)(dir + 0.5) & 7);
}
...
#define HATEEDGE 3
	...
	while (maxoo) {
		...
		if (isok(x, y) && levl[x][y].typ != fg) {
			...
			ky[cnt] = y;
			/* diagonal */
			if (!diagonal && vx[j] && vy[j]) continue;
