...
int *fail_reason;
{
	...
	if ((x == u.ux && y == u.uy) || cause == ANIMATE_SPELL) {
		...
		else pline_The("ice-frozen corpse disappears!");
	    } else {
		if (cause == ANIMATE_SPELL)
	    ...
	    pline("Instead of %sing, the %s suddenly %s!",
		frozen ? "melt" : "shatter",
		frozen ? "corpse": "statue",
		canspotmon(mon) ? "comes to life" : "disappears");
	else {/* cause == ANIMATE_NORMAL */
	    You("find %s posing as %s.",
		canspotmon(mon) ? a_monnam(mon) : something,
		frozen ? "frozen" : "a statue");
	    stop_occupation();
	}
...
unsigned trflags;
{
	...
	switch(ttype) {
		...
		if (u.umonnum == PM_IRON_GOLEM) {
		    ...
		    pline("%s you!", A_gush_of_water_hits);
		    losehp(rnd(20), "gush of water", KILLED_BY_AN);
		    break;
		}
...
glovecheck:		(void) rust_dmg(uarmg, "gauntlets", 1, TRUE, &youmonst);
		...
		if (u.uspellprot) {
		    pline_The("%s haze around you vanishes away!", hcolor(NH_GOLDEN));
		    u.usptime = u.uspmtime = u.uspellprot = 0;
		    find_ac();
		}
		if (u.uspellit) {
		    extinguish_torch();
		    Your("magic torch is extinguished!");
		}
		break;
...
register struct monst *mtmp;
{
	...
	if (!trap) {
...
#ifdef MONSTEED
			if (mtmp->mriding && rn2(3)) {
			    thitm(8, mtmp->mchild, otmp, 0, FALSE);
...
#ifdef MONSTEED
			if (mtmp->mriding && rn2(3)) {
			    thitm(7, mtmp->mchild, otmp, 0, FALSE);
			    trapkilled = DEADMONSTER(mtmp);
			} else
#endif
			if (thitm(7, mtmp, otmp, 0, FALSE)) trapkilled = TRUE;
		...
		case SLP_GAS_TRAP:
#ifdef MONSTEED
		    if (mtmp->mriding) mintrap(mtmp->mchild);
#endif
		    if (!resists_sleep(mtmp) && !breathless(mptr) &&
				!mtmp->msleeping && mtmp->mcanmove) {
			    mtmp->mcanmove = 0;
...
#ifdef MONSTEED
			if (mtmp->mriding) {
			    if (!thitm(0, mtmp->mchild, (struct obj *)0, rnd(16), FALSE))
				mintrap(mtmp->mchild);
			    trapkilled = DEADMONSTER(mtmp);
...
struct trap *ttmp;
{
	...
	obj = getobj(oil, "untrap with", getobj_filter_untrap_squeaky);
	if (!obj) return 0;
...
#ifdef MONSTEED
	if (mtmp->mriding) wt += mtmp->mchild->data->cwt;
#endif
...
#ifdef MONSTEED
	if (mtmp->mriding)
	    for (otmp = mtmp->mchild->minvent; otmp; otmp = otmp->nobj)
		wt += otmp->owt;
#endif
...
lava_effects()
{
	...
	You("slowly rise up above the lava.");
	u.utrap = 0;
	u.utraptype = 0;
    }
