...
//STATIC_PTR int NDECL(do_vorpalize); /* test code */
//#ifdef AUTOTHRUST
STATIC_DCL int NDECL(dovattack);			/* autothrust */
STATIC_DCL int NDECL(autothrust);			/* autothrust */
STATIC_DCL struct monst* FDECL(findtarget, (int, int));	/* autothrust */
STATIC_DCL struct monst* FDECL(autotarget, (int));	/* autothrust */
//#endif
...
#define YOURESIST(x,y) you_are((is_full_resist(y) ? "protected from %s" : " resistant"), (x))
...
struct ext_func_tab extcmdlist[] = {
	...
	{"version", "list compile time options for this version of NetHack",
		doextversion, TRUE},
//	{"vorpalize", "vorpalize weapon", do_vorpalize, FALSE}, /* debug */
//	{"wepskill", "show weapon skill", show_weaponskill, FALSE}, /* debug */
...
// test code
//
//static NEARDATA const char fuse_objs[] =
//	{ ALL_CLASSES, ALLOW_NONE, WAND_CLASS, RING_CLASS, 0 };
//
//int
//do_vorpalize()
//{
//  register struct obj *wep, *fuseobj;
//  
//  if (!uwep) {
...
//  }
//  if (!uwep->onamelth) {
//    You("need to name your weapon first.");
//    return(0);
//  }
...
//    
//    if (!(fuseobj = getobj(fuse_objs, "fuse to")))
//      /* Cancelled */
//      return (0);
//    if (vorpalize_weapon(wep, fuseobj)) {
//      useup(fuseobj);
//      Your("weapon seems to be sharper.");
//      if (carried(wep)) update_inventory();
//    } else {
//      You("cannot vorpalize your weapon with that!");
//    }
//  }
//  return(0);
//}
...
dovattack()
{
	...
	if (uwep && is_ranged(uwep)) return autothrust();
	pline("Your wielding weapon has no other use than a melee combat.");
	return 0;
}
...
int x, y;
{
	...
	if (couldsee(x, y) &&			/* your weapon should not be blocked */
	    mtmp && canspotmons(mtmp) &&	/* you can see the target */
...
autothrust()
{
	...
	if (u.ulasttgt) {		/* if last target is alive and in range, attack it */
		...
		y = u.ulasttgt->my;
		mtmp = findtarget(x, y);
		if (mtmp && distu(x, y) <= weapon_range(uwep)) {
#ifdef MONSTEED
			mtmp = target_rider_or_steed(&youmonst, mtmp);
#endif /*MONSTEED*/
			(void) attack(mtmp);
			u.dx = sgn(x - u.ux); /* hero's direction */
			u.dy = sgn(y - u.uy);
			u.dz = 0;
			return TRUE;
		}
	}						/* else find another target */
	if (mtmp = autotarget(weapon_range(uwep))) {
#ifdef MONSTEED
		mtmp = target_rider_or_steed(&youmonst, mtmp);
#endif /*MONSTEED*/
		(void) attack(mtmp);
		u.dx = sgn(mtmp->mx - u.ux); /* hero's direction */
		...
		You("don't see any target in range.");
		return FALSE;
	}
	return TRUE;
}
...
playersteal()
{
	...
	if(MON_AT(x, y)) {
		...
		otmp = display_minventory(mtmp, MINV_ALL, "Which item do you want to steal?");
		if (!otmp) {
		    You("%s nothing.", verb);
		    goto cleanup;
		} else {
...
#endif /*WIZARD*/
		    if (owt > 190) owt = 190;
		    if (ch < owt*2 || !mtmp->mpeaceful) found = TRUE;
...
#endif
			{
				...
				You(stealmsg, verb, doname(otmp),
				    mon_nam(mtmp), ", but cannot hold it!");
				found = TRUE;
			    }
			}
		    }
...
specialpower()      /* Special class abilites */
{
	...
	if (u.unextuse) {
		...
		if(Hallucination || Stunned || Confusion) {
		    You("don't quite feal up to studing right now.");
		    break;
		} else if((ACURR(A_INT) + ACURR(A_WIS)) < rnd(60)) {
		    You("try to study but get distracted, try again later.");
		...
		if(!uwep) {
		    ...
		    You("already know what that weapon's quality!");
		    break;
		} else if(is_missile(uwep) || is_ammo(uwep)) {
		    You("can't practice with that!");
		    break;
		} else {
		    You("study and practice with your weapon.");
		    if (rnd(15) <= ACURR(A_INT)) {
			You("were able to descern it's general quality.");
			makeknown(uwep->otyp);
			uwep->known = TRUE;
			prinv((char *)0, uwep, 0L);
		    } else 
			pline("Unfortunatly, you still don't know it's quality.");
		}
		u.unextuse = rn1(250,250);
		break;
		...
		if (Sick || Slimed) {
		    if(carrying(SCALPEL)) {
			pline("Using your scalpel, you cure your infection!");
		...
		if (u.uhp < u.uhpmax) {
		    ...
		    u.unextuse = rn1(1000,500);
		    flags.botl = 1;
		} else pline("Don't need healing!");
		break;
	    case PM_KNIGHT: 
		if (u.uhp < u.uhpmax || Sick) {
			You("lay your hands on your wounds and pray.");
