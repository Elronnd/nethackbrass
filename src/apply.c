...
dorub()
{
	struct obj *obj = getobj(cuddly, "rub", getobj_filter_rub);
...
//	if (obj != uwep) {
//	    if (!wield_tool(obj, 0)) return 0;
//	    /* prevent bashing msg */
//	    unweapon = FALSE;
//	}
	otmp = getobj(clothonly, "cut", getobj_filter_scissors);
	if (!otmp) return 0;
	...
	if (!Role_if(PM_HEALER) &&
	    ...
	    if (cnt && ACURR(A_DEX) < rnd(18)) cnt = rn2(cnt);
	}
	if (cnt <= 0) {
	    You("cut your %s in sunder.", xname(otmp));
	    useup(otmp);
	    update_inventory();
	    return 1;
	}
...
putbandage(void)		/* called during each move whilst putting a bandage */
{
	...
	if(bandage.usedtime++ >= 50) {
		You("give up your attempt to put the bandage.");
		return(0);
	}
	if(bandage.usedtime < bandage.reqtime)
		return(1);		/* still busy */
	You("finish first aid.");
	if (!otmp->cursed || !rn2(3)) {
		You_feel("better.");
		healup(d(4 + 2 * bcsign(otmp), 4), 0, FALSE, FALSE);
	} else {
	    const char *mp = "medical malpractice";
	    if (!Sick_resistance && !rn2(13)) {
		pline("The bandage is badly contaminated!");
		make_sick(Sick ? Sick/3L + 1L : (long)rn1(ACURR(A_CON),20),
			  mp, TRUE, SICK_NONVOMITABLE);
	    } else {
		pline("The bandage must be unclean!");
		You_feel("sick.");
...
#define ATTR_COUNT (A_MAX*3)	/* number of attribute points we might fix */
	...
	if (obj && obj->cursed) {
	    ...
	    if (!rn2(5) && uwep != obj) {
		Your("unicorn horn disintegrates.");
		useup(obj);
	    }
	    return;
	}
...
//    else if (ACURR(A_DEX) >= 14) proficient += (ACURR(A_DEX) - 14);
	...
	if (otmp) {
		...
		if (!vanish_ether_obj(otmp, (struct monst *)0, "snatch"))
...
use_pole (obj)
	struct obj *obj;
{
	int res = 0, max_range = 4, min_range = 4;
	coord cc;
	struct monst *mtmp;
...
struct obj *obj;
{
	...
	if (cnt) {
		...
		Sprintf(buf, "Bullets loaded in the %s:", xname(obj));
		putstr(win, 0, buf);
		putstr(win, 0, "");
		...
		pline("No bullet is loaded in your %s.", xname(obj));
	}
	...
	/* Get bullets to load */
	otmp = getobj(bullets, "load with", getobj_filter_bullet);
	if(!otmp) return 0;
	if (!is_bullet(otmp)) {
	...
	You("load your %s with %s.", xname(obj), doname(otmp));
...
/* STEPHEN WHITE'S NEW CODE */
	...
	case BANDAGE:
		if (u.uhp < u.uhpmax) {
			...
			bandage.usedtime = 0;
			bandage.bandage = obj;
			set_occupation(putbandage, "putting the bandage", 0);
			You("start putting a bandage on your wounds.");
		} else {
			You("are not wounded.");
			res = 0;
		}
