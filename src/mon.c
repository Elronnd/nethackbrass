...
register struct monst *mtmp, *mtmp2;
{
	...
	if (mtmp3->mparent == mtmp) mtmp3->mparent = mtmp2;
	if (mtmp3->mchild  == mtmp) mtmp3->mchild  = mtmp2;
    }
...
#ifdef MONSTEED
	if (!mon->msteed) /* cancel remove_monster() for msteed */
#endif
...
struct permonst *mptr;	/* reflects mtmp->data _prior_ to mtmp's death */
{
	...
	relobj(mtmp, 0, FALSE);
#ifdef MONSTEED
	if (!mtmp->msteed) /* cancel remove_monster() for msteed */
#endif
...
register struct monst *mtmp;
{
	...
	if (msvv) {
		...
		if (cansee(x,y))
		    pline("%s falls off of %s steed!", Monnam(msvv),
			humanoid(msvv->data) ? (msvv->female ? "her" : "his") : "its");
		(void) mintrap(msvv);
		(void) minliquid(msvv);
	    }
	}
...
register struct monst *mdef;
{
	...
	if (mdef->mriding) {
	    mdef->mriding = 0;
	    mdef->mchild->msteed = 0;
	    mongone(mdef->mchild);
	} else if (mdef->msteed) {
	    mdef->msteed = 0;
	    mdef->mparent->mriding = 0;
	    mongone(mdef->mparent);
	}
#endif
...
//	if(is_human(mdat) && (!always_hostile(mdat) && mtmp->malign <= 0) &&
	if(your_race(mdat) && (!always_hostile(mdat) && mtmp->malign <= 0) &&
...
boolean pf;
{
	...
	if (mtmp->mriding) setmpeaceful_core(mtmp->mchild, pf);
	else if (mtmp->msteed) setmpeaceful_core(mtmp->mparent, pf);
#endif
}
...
/* and the same for maximum hit points */
	...
	rel_ether_gear(mtmp);
#ifdef MONSTEED
	if ((mtmp->mriding && !mcanride(mtmp, mtmp->mchild)) ||
	    (mtmp->msteed && !mcanride(mtmp->mparent, mtmp)))
